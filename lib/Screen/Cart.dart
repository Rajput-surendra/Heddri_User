...
bool? isTimeSlot,
    ...
    isUseWallet = false,
    isPayLayShow = true;
int? selectedTime, selectedDate = -1, selectedMethod;
                                                  ...
                                                  ? () async{
                                                      if(ordertype == null || ordertype == ""
                                                        ...
                                                        msg = getTranslated(
                                                            context,
                                                            'selectType');
                                                        ordertype =  await Navigator.push(context, MaterialPageRoute(builder: (context)=>OrderType()));
                                                        checkoutState!(() {
                                                          _placeOrder = true;
                                                        });
                                                      }
                                                      else if (selAddress == null ||
                                                          selAddress!.isEmpty) {
                                                        msg = getTranslated(
                                                            context,
                                                            'addressWarning');
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            InkWell(
              onTap: () async {
                ordertype =  await Navigator.push(context,
                    MaterialPageRoute(builder: (context) => OrderType()));
                print("Value@@@ ${ordertype}");
              },
              child: Row(
                children: [
                  ...
                  Padding(
                      padding: const EdgeInsetsDirectional.only(start: 8.0),
                      child: Text(
                        getTranslated(context, 'ORDER_TYPE') ?? '',
                        style: TextStyle(
                            fontWeight: FontWeight.bold,
                            color: Theme.of(context).colorScheme.fontColor),
                      )),
            ...
            Divider(),
             ordertype == null || ordertype == "" ? SizedBox.shrink() : Text("${ordertype}"),
          ],
        ),
      ),
    );
