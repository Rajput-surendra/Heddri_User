...
import 'package:eshop_multivendor/Model/OfferModel.dart';
import 'package:eshop_multivendor/Model/SecondBannerModel.dart';
...
class _HomePageState extends State<HomePage>
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    //   child: Image.asset("assets/images/ad-banner3.jpg"),
                    // ),
                 //  _exploreByCategory(),
                    SizedBox(height: 20,)
                    // Container(
                    //   padding: EdgeInsets.all(10.0),
                    //   child: Image.asset("assets/images/ad-banner2.png"),
                    // ),
                   // offerList.length>0?_offer():SizedBox(height: 30,),
                   // _seller()
                  ],
                ),
              ),
  ...
  _getVarient(model,int? pos) {
    if (model.type == "variable_product") {
      List att = [], val = [];
      ...
      return model.prVarientList![model.selVarient!]
          .attr_name !=
          null &&
          ...
          children: [
            ...
            Container(
              ...
              child: GridView.builder(
                  ...
                  itemCount: catList.length < 6 ? catList.length : 6,
                  itemBuilder: (context, index) {
                    return Padding(
                      padding: const EdgeInsetsDirectional.only(end: 10),
                      child: GestureDetector(
                        onTap: () async {
                          if(catList[index].name == "Carrier"){
                            Navigator.push(context, MaterialPageRoute(builder: (context)=>CarrierCategeory()));
                          }else{
                            if (catList[index].subList == null ||
                                catList[index].subList!.length == 0) {
                              print("explore Category home page id =================================>: ${catList[index].id}");
                ...
                child: Column(
                  ...
                  children: <Widget>[
                   ...
                   // _getHeading(sectionList[index].title ?? "", index),
                    ...
                    sectionList[index].title == "Today Offer"
                        ? FutureBuilder(
                            ...
                            builder:
                              ...
                              SecondBannerModel bannerModel = snapshot.data;
                              if (snapshot.hasData) {
                                print("IMAges$imageUrl${bannerModel.data![0].image}");
                                return bannerModel.error == true ? Container(
                                  child: Image.network(
                                      "$imageUrl${bannerModel.data![0].image}"),
                    ...
                    sectionList[index].title == "Seasonal Offers" ?
                    _getSection(index): SizedBox(),
                  ],
                ),
              ),
        ...
        child: InkWell(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              _getVarient(sectionList[secPos]
  ...
  Widget offerProductItem(int secPos) {
    String? offPer;
    double price = double.parse(
        offerList[secPos].data.product[0].prVarientList![0].disPrice!);
    if (price == 0) {
      price = double.parse(
          offerList[secPos].data.product[0].prVarientList![0].price!);
    } else {
      double off = double.parse(offerList[secPos].data.product[0]
          .prVarientList![0]
          .price!) -
          price;
      offPer = ((off * 100) /
          double.parse(offerList[secPos].data.product[0]
              .prVarientList![0]
              .price!))
          .toStringAsFixed(2);
    }
    return Card(
      ...
      child: InkWell(
        ...
        child: Column(
          ...
          children: <Widget>[
                ...
                children: [
                  ClipRRect(
                    ...
                    child: Hero(
                      transitionOnUserGestures: true,
                      tag:
                      "${offerList[secPos].data.product[0].id}$secPos",
                      child: FadeInImage(
                        fadeInDuration: Duration(milliseconds: 150),
                        image: CachedNetworkImageProvider(
                            offerList[secPos].data.product[0].image!),
                        height: double.maxFinite,
                        width: double.maxFinite,
                        imageErrorBuilder: (context, error, stackTrace) =>
                            erroWidget(double.maxFinite),
            ...
            Padding(
              ...
              child: Text(
                offerList[secPos].data.product[0].name!,
                style: Theme.of(context).textTheme.caption!.copyWith(
                    color: Theme.of(context).colorScheme.lightBlack),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
            ...
            _getVarient(offerList[secPos].data.product[0], 0),
            Text(
              " " + CUR_CURRENCY! + " " + price.toString(),
              style: TextStyle(
                color: Theme.of(context).colorScheme.fontColor,
            ...
            Padding(
              padding: const EdgeInsetsDirectional.only(
                  start: 5.0, bottom: 5, top: 3),
              child: double.parse(offerList[secPos].data.product[0]
                  .prVarientList![0]
                  .disPrice!) !=
                  0
                  ? Row(
                children: <Widget>[
                  Text(
                    double.parse(offerList[secPos].data.product[0]
                        ...
                        ? CUR_CURRENCY! +
                        "" +
                        offerList[secPos].data.product[0]
                            .prVarientList![0]
                            .price!
                        : "",
                    style: Theme.of(context)
        ...
        onTap: () {
          Product model = offerList[secPos].data.product[0];
          Navigator.push(
            context,
            PageRouteBuilder(
              // transitionDuration: Duration(milliseconds: 150),
              pageBuilder: (_, __, ___) => ProductDetail(
                  model: model, secPos: secPos, index: 0, list: false
                //  title: sectionList[secPos].title,
              ),
            ),
          );
      ...
      builder: (context, data, child) {
        return data
            ...
            : ListView.builder(
                ...
                physics: NeverScrollableScrollPhysics(),
                itemBuilder: (context, index) {
                  print("LENGHT: ${sectionList.length}");
    ...
    return Selector<HomeProvider, bool>(
      builder: (context, data, child) {
        return data
          ...
          child: ListView.builder(
            itemCount: catList.length <= 4 ? catList.length : 4,
            scrollDirection: Axis.horizontal,
            shrinkWrap: true,
            physics: AlwaysScrollableScrollPhysics(),
            itemBuilder: (context, index) {
                  ...
                  child: GestureDetector(
                    onTap: () async {
                      SharedPreferences prefs = await SharedPreferences.getInstance();
                        prefs.setString('catName', '${catList[index].name}');
                    //  print("checking categoru category sub list ${catList[index].subList} and ${catList[index].subList!.length}");
                     if(catList[index].name == "Carrier"){
                       Navigator.push(context, MaterialPageRoute(builder: (context)=>CarrierCategeory(bannerCarrier: catList[index].banner,)));
                     }else{
                       if (catList[index].subList == null ||
                           catList[index].subList!.length == 0) {
                         print("Category home page id ========================================>: ${catList[index].id} and ${catList[index].name}");
                         await Navigator.push(
                             context,
                             MaterialPageRoute(
                               builder: (context) => ProductList(
 ...
 /* _catList() {
      ...
      builder: (context, data, child) {
        return data
            ...
            : Container(
                ...
                child: ListView.builder(
                  itemCount: catList.length <= 4 ? catList.length : 5,
                  scrollDirection: Axis.horizontal,
                  shrinkWrap: true,
                  physics: AlwaysScrollableScrollPhysics(),
                  itemBuilder: (context, index) {
                      ...
                      return Padding(
                        ...
                        child: GestureDetector(
                          onTap: () async {
                            if (catList[index].subList == null ||
                                catList[index].subList!.length == 0) {
                              print("Category home page id ========================================>: ${catList[index].id}");
                              await Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => ProductList(
  ...
  void getProduct() {
    ...
    apiBaseHelper.postAPICall(Uri.parse('${baseUrl}get_offer_images'), {}).then((getdata) {
      var data = getdata["data"];
      setState(() {
        offerList = (data as List)
            .map((data) => new NewOfferModel.fromJson(data))
            .toList();
      });
  ...
  void getOfferImages() {
    ...
    if(false){
      offerImages.clear();
      offerImages = (data as List).map((data) => new Model.fromSlider(data)).toList();
  ...
  void getSection() {
    Map parameter = {PRODUCT_LIMIT: "10", PRODUCT_OFFSET: "0"};
  ...
  void getCat() {
    ...
    print("category api here ${getCatApi} and ${parameter}");
    apiBaseHelper.postAPICall(getCatApi, parameter).then((getdata) {
      ...
      if (!error) {
        ...
        catList =
          ...
          print("checking cat list here ${catList[0].id}");
