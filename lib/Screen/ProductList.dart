...
class StateProduct extends State<ProductList> with TickerProviderStateMixin {
  ...
  void getProduct(String top) {
    ...
    print("get product response here ${getProductApi} and ${parameter}");
    apiBaseHelper.postAPICall(getProductApi, parameter).then((getdata) {
      bool error = getdata["error"];
      String? msg = getdata["message"];
      if (!error) {
        print("filter product parameter =========================> $parameter");
              ...
              child: Column(
                children: [
                  ...
                  widget.subList != null
                      ...
                      : SizedBox(),
                  widget.subList != null && widget.subList.length > 0
                      ? ListView.builder(
                          itemCount: widget.subList.length,
                          shrinkWrap: true,
                          itemBuilder: (context, index) {
                            return widget.subList[index].subList == null ||
                                    widget.subList[index].subList.length == 0
                                ? InkWell(
                                    onTap: () {
                                      setState(() {
                                        widget.id = widget.subList[index].id;
                                        widget.name =
                  ...
                  // Wrap(
                  //   spacing: 10,
                  //   children: catList.asMap().entries.map<Widget>((e) =>
                  //       InkWell(
                  //         onTap: (){
                  //           fProductId = e.value.categoryId ;
                  //           print("Filter Product Id ==\\\\=====> $fProductId");
                  //
                  //           if (mounted)
                  //             setState(() {
                  //               if (isSelected == true) {
                  //                 selectedId.add(fProductId);
                  //               } else {
                  //                 selectedId.remove(fProductId);
                  //               }
                  //             });
                  //         },
                  //         child: Chip(label: Text("${e.value.name}",
                  //             style: Theme.of(context)
                  //                 .textTheme
                  //                 .subtitle1!
                  //                 .copyWith(
